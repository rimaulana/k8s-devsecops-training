...
puproject = os.environ['CODEBUILDZPPROJECT']
...
def handler(event, context):
  ...
  if event['detail']['project-name'] in [puproject]:
    errors = '## Dynamic Analysis - App Scanning (using ZAP)\n'
    if event['detail']['build-status'] == 'SUCCEEDED':
      errors = errors + 'App passed ZAP dynamic analysis'
    else:
      errors = errors + 'App failed ZAP dynamic analysis.  Please review the logs'
